...
import { MessageType, PageSignature, SendResponseType, SessionData, StoredSession } from "@shared/types";
...
export class MessageService {
  ...
  private async processMessage(message: MessageType, sendResponse: SendResponseType): Promise<void> {
    switch (message.action) {
      ...
      case MESSAGE_ACTIONS.GET_PAGE_SIGNATURE:
        await this.handleGetPageSignature(message, sendResponse);
        break;
      ...
      case MESSAGE_ACTIONS.VALIDATE_SESSIONS:
        await this.handleValidateSessions(message, sendResponse);
        break;
  ...
  private async handleGetPageSignature(
    message: Extract<MessageType, { action: "getPageSignature" }>,
    sendResponse: SendResponseType<PageSignature | null>
  ): Promise<void> {
    const signature = await this.sessionHandler.getPageSignature(message.tabId);
  ...
  private async handleValidateSessions(
    message: Extract<MessageType, { action: "validateSessions" }>,
    sendResponse: SendResponseType<SessionData[]>
  ): Promise<void> {
    const validatedSessions = await this.sessionHandler.validateSessions(message.sessions);
    sendResponse({ success: true, data: validatedSessions });
  }
}
